PROGRAM MAIN
  ...
  do ifile=1,2
     IF(ifile.EQ.1)THEN        
        OPEN(UNIT=fileLu,FILE='Calpha1.data',STATUS='UNKNOWN',&
...
!        nocctot = 99
     ELSE
        OPEN(UNIT=fileLu,FILE='Calpha2.data',STATUS='UNKNOWN',&
             & FORM='UNFORMATTED',IOSTAT=IOS)
...
!        nocctot = 99
     ...
     write(fileLu) nvirt,nocc,noccEOS,nvirtEOS,NBA,nocctot

     allocate(Calpha(NBA,nvirt,nocc))
     do C=1,nocc
        do B=1,nvirt
           do A=1,NBA
              Calpha(A,B,C) = A*0.00124235636_8/A + B*0.01010101_8/B + C*0.014321_8/C
           enddo
        enddo
     enddo
     write(fileLu) Calpha
     allocate(EVocc(nocc))
     do C=1,nocc
        EVocc(C) = C*1.934321_8/C
     enddo
     write(fileLu) EVocc

     allocate(EVvirt(nvirt))
     do C=1,nvirt
        EVvirt(C) = C*1.91321_8/C
     enddo
     write(fileLu) EVvirt
     ...
     do B=1,noccEOS
        do A=1,nocc
           UoccEOST(A,B) = A*0.030321_8/A + B*0.101121_8/B
        enddo
     enddo
     ...
     do B=1,nvirt
        do A=1,nvirt
           UvirtT(A,B) = A*0.021321_8/A + B*0.01321_8/B
        enddo
     enddo
     write(fileLu) UvirtT   
     ...
     do B=1,nocc
        do A=1,nocc
           UoccT(A,B) = A*0.032321_8/A + B*0.01521_8/B
        enddo
     enddo
     write(fileLu) UoccT        
     ...
     do B=1,nvirtEOS
        do A=1,nvirt
           UvirtEOST(A,B) = A*0.023321_8/A + B*0.01421_8/B
        enddo
     enddo
     ...
     IF(first_order)THEN
        ...
        do C=1,nocctot
           do B=1,nocc
              do A=1,NBA
                 CalphaOcc(A,B,C) = A*0.01124235636_8 + B*0.01011111_8 + C*0.16422_8
              enddo
           enddo
        enddo
        write(fileLu) CalphaOcc
        ...
        do C=1,nvirt
           do B=1,nvirt
              do A=1,NBA
                 CalphaVV(A,B,C) = A*0.01124235636_8 + B*0.01011111_8 + C*0.16422_8
              enddo
           enddo
        enddo
        write(fileLu) CalphaVV
